...
def center_error(recovered, true):
    ...
    """
    return np.mean([
        min(np.linalg.norm(r - t) for t in true)
        for r in recovered
    ])
...
while running:
    ...
    for i in range(0, 101, 10):
        pygame.draw.line(screen, (235,235,235), to_screen([i,0]), to_screen([i,100]))
        pygame.draw.line(screen, (235,235,235), to_screen([0,i]), to_screen([100,i]))

        screen.blit(font.render(str(i), True, (0,0,0)),
    ...
    for c in true_centers:
        pygame.draw.circle(
            screen, (200,200,200), to_screen(c),
            int(CLUSTER_RADIUS * SCALE), 1
        )
    ...
    if not completed:
        ...
        screen.blit(font.render(f"P = {path_P:.1f} m", True, (0,0,0)), (px + 10, py - 20))
        screen.blit(font.render(f"C = {cost_C:.1f} J", True, (0,0,0)), (px + 10, py - 6))
