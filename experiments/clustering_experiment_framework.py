...
def run_clustering(method,X):
    ...
    if method=="kmeans":
        ...
        labels=m.fit_predict(X)
        centers=np.array([X[labels==i].mean(axis=0) for i in set(labels) if i!=-1])
...
def to_screen(p): return int(p[0]*SCALE),int((MAP_SIZE-p[1])*SCALE)
...
def path_len(p): return sum(np.linalg.norm(p[i]-p[i+1]) for i in range(len(p)-1))
best=None; cost=float("inf")
for perm in itertools.permutations(centers):
    p=[DCC]+list(perm)+[DCC]
    c=path_len(p)
    if c<cost: cost=c; best=np.array(p)

# =========================================================
# UAV
# =========================================================
uav=best[0].copy(); idx=1; speed=0.5
...
while running:
    ...
    for i in range(len(best)-1):
        pygame.draw.line(screen,(120,120,120),to_screen(best[i]),to_screen(best[i+1]),2)
