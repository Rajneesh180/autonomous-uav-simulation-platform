...
| Rendezvous Point Selection | `{Config.ENABLE_RENDEZVOUS_SELECTION}` |
| Node Energy Depletion | `{Config.ENABLE_NODE_ENERGY_DRAIN}` |
| Multi-Trial Sensing | `{Config.ENABLE_PROBABILISTIC_SENSING}` |
| GA Sequence Optimizer | `{Config.ENABLE_GA_SEQUENCE}` |
| 3D Gaussian Obstacles | `True` |
| Agent-Centric Transform | `True` |
| TDMA Scheduling | `{Config.ENABLE_TDMA_SCHEDULING}` |
| BS Uplink Model | `{Config.ENABLE_BS_UPLINK_MODEL}` |
| SCA Hover Optimizer | `{Config.ENABLE_SCA_HOVER}` |
